    ...
    useEffect(() => {
        ...
        socket.on("poll-started", ({ poll, remaining }) => {
            ...
            navigate("/live-results", {
                state: {
                    poll,
                    timeLimit: remaining || poll.timeLimit,
                },
            });
        });
            ...
            <div className="max-w-6xl mx-auto p-8 bg-white rounded-xl shadow border border-gray-200">
                ...
                <div className="flex justify-end">
                    <button
                        onClick={createPoll}
                        disabled={!canAskNew}
                        className={`w-full max-w-xs font-semibold py-2 rounded-full transition-all ${canAskNew ? "bg-primary text-white hover:bg-purple-700" : "bg-gray-300 text-gray-600 cursor-not-allowed"}`}
                    >
                        Ask Question
                    </button>
                </div>
