...
const StudentDashboard = () => {
    ...
    useEffect(() => {
        socket.on('kicked', () => {
            ...
            sessionStorage.removeItem("userRole"); // clear per-session student identity
            navigate('/kicked');
        });
    ...
    const handleRegister = () => {
        ...
        socket.once("registration:success", () => {
            // persist per-tab so other tabs can register independently
    ...
    useEffect(() => {
        socket.on("poll-started", ({ poll, remaining }) => {
            console.log("Received poll start:", poll, "remaining", remaining);
            setQuestion(poll);
            setSubmitted(false);
            setResult(null);
            setSelectedOption("");
            setTimer(remaining || poll.timeLimit || 60);
        });
        ...
        socket.on("already-answered", ({ selectedOption }) => {
            if (selectedOption) {
    ...
    // whenever serverTimer updates copy to local
    useEffect(() => {
